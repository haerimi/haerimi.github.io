...
초기 가이드 버튼 UI를 구현하고, 발신자(사용자/AI)에 따라 말풍선 위치가 달라지는 공통 메시지 컴포넌트를 설계해 중복 코드를 최소화했습니다.
...
`,
            ...
            features: [
                ...
                {
                    title: '맞춤형 상품 추천',
                    description: '사용자 응답 데이터를 기반으로 여행 상품을 추천하는 기능을 구현했습니다.',
                    image: pengdi3,
                    webpSrc: pengdi4
                }
            ],
...
`,
            ...
            features: [
                ...
                {
                    title: 'PINI 실행 대기',
                    description: '모든 참가자의 선호가 저장되면 진행 바가 완료되고 호스트에게 PINI 실행 버튼이 활성화됩니다. 폴링으로 참가자 완료 여부를 감지하되, 해당 페이지에서만 동작하고 탭 이탈 시 중단·복귀 시 재개되도록 구현했습니다.',
                    image: cluepot2,
                },
                {
                    title: 'PINI 장소 추천',
    ...
    },
    {
        ...
        details: {
            overview: {
                description: `
### 🚀 개요
반복적인 데이터 수집 작업을 자동화하고 주요 지표를 Slack으로 자동 전송하는 리포트 시스템을 구축했습니다.

일별 판매 지표는 Slack 메시지로, 월별 집계 데이터는 ExcelJS로 가공한 엑셀 파일로 전송되도록 구분해 구현했습니다.

TypeORM에서 기본 ORM과 쿼리 빌더를 상황에 맞게 선택해 사용했습니다. 판매 상품·카테고리 등 복잡한 집계는 UNION·GROUP BY를 쿼리 빌더로 직접 작성해 성능을 확보했습니다.

Cloud Scheduler로 매일 정해진 시간에 자동 실행되도록 설정해, 수동으로 처리하던 반복 작업을 완전히 자동화했습니다.
`,
                image: codeImg
            },
            features: [
